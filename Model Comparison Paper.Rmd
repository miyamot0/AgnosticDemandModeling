...
dataFramePrep = data.frame(
  ...
  q0.HSw2  = numeric(length = nParticipants),
  ...
  a.HSw2   = numeric(length = nParticipants),
...
for (id in 1:nParticipants) {
  ...
  fit.Koffa <- optim(par    = c(fit.Koff$par[1], fit.Koff$par[2]),
                     fn     = min.RSS.EXPT.aw,
                     method = "L-BFGS-B",
                     lower  = c(0.01, -6),
  ...
  fit.HWa <- optim(par    = c(fit.Koff$par[1], fit.Koff$par[2]),
                   fn     = min.RSS.EXPL.aw,
                   method = "L-BFGS-B",
                   lower  = c(0.01, -5),
                   upper  = c(max(currentData$y) * 1.25, 0),
  ...
  fit.HWb <- optim(par    = c(fit.Koff$par[1], fit.Koff$par[2]),
                   fn     = min.RSS.EXPL.aw2,
                   method = "L-BFGS-B",
                   lower  = c(0.01, -5),
                   upper  = c(max(currentData$y) * 1.25, 0),
...
```{r s2vs3fig, fig.cap="Comparisons of Strategy 2 and 3 (Relative Error)", fig.height=8}
...
tost(frameToAnalyze$q0.HSw2, 
     frameToAnalyze$q0.Koffw, 
     epsilon = 1, 
     conf.level = 0.9875,
     paired = TRUE)

cor.2v3.q0 = cor.test(frameToAnalyze$q0.Koffw, 
                      frameToAnalyze$q0.HSw2)

tost(frameToAnalyze$a.Koffw, 
     frameToAnalyze$a.HSw2, 
     epsilon = 1, 
     conf.level = 0.9875,
     paired = TRUE)
...
#comps.base <- lm(q0.Koffw ~ q0.HS, data = frameToAnalyze)
plot(frameToAnalyze$q0.Koffw ~ frameToAnalyze$q0.HSw2,
     main = expression(Q[0] ~ "Estimates"),
     col  = alpha("black", 0.1),
     ylab = "Strategy 2",
     ylim = c(0,100),
...
#comps.base <- lm(a.Koffw ~ a.HS, data = frameToAnalyze)
plot(frameToAnalyze$a.Koffw ~ frameToAnalyze$a.HSw2,
     main = expression(alpha ~ "Estimates"),
     col  = alpha("black", 0.1),
     ylab = "Strategy 2",
     ylim = c(-5,0),
...
write.csv(frameToAnalyze, file = "outputfile.csv")
...
Evaluations of the relationship between Strategy 2 and 4 revealed strong, but not perfect correlations for $Q_0$ (r=`r cor.2v3.q0$estimate`, t=`r cor.2v3.q0$statistic`, df=`r cor.2v3.q0$parameter`, `r scales::pvalue(cor.2v3.q0$p.value, accuracy = .025, add_p = TRUE)`) and for $\alpha$ (r=`r cor.2v3.a$estimate`, t=`r cor.2v3.a$statistic`, df=`r cor.2v3.a$parameter`, `r scales::pvalue(cor.2v3.a$p.value, accuracy = .025, add_p = TRUE)`). Despite strong correlations, the results of TOSTs for not significant for $Q_0$ (`r scales::pvalue(tost.2v3.q0$tost.p.value, accuracy = .025, add_p = TRUE)`) but significant for $\alpha$ (`r scales::pvalue(tost.2v3.a$tost.p.value, accuracy = .025, add_p = TRUE)`).

That is, the results of equivalence testing indicated that both strategies yielded statistically equivalent model parameters for both $Q_0$ and $\alpha$. That is, results of equivalence testing rejected the null hypothesis of statistical difference and indicated that the estimates yielded from each strategy were statistically equivalent for $\alpha$ but and for $Q_0$. A visualization of these relationships are illustrated in \autoref{fig:s2vs3fig}.
