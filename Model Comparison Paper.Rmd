---
title             : "Splitting Hairs in the Operant Demand Framework: Accomodating Zeroes in Demand Curve Analysis"
...
The data generating process produced a total of `r nrow(frameToAnalyze)` series that met all 3 criteria for systematic hypothetical purchase task data (from *N*=`r nParticipants`; `r round((nrow(frameToAnalyze)/nParticipants) * 100, 2)`%). The correspondence between parameter estimates are illustrated using scatterplot visualizations in \autoref{fig:fig4}. The relationships illustrated in \autoref{fig:fig4} indicates varying degrees of correspondence and correspondence between each of the strategies are discussed in greater detail below.
...
An evaluation of the relationship between Strategy 1 and 2 revealed strong correlations for $Q_0$ (r=`r cor.1v2.q0$estimate`, t=`r cor.1v2.q0$statistic`, df=`r cor.1v2.q0$parameter`, `r scales::pvalue(cor.1v2.q0$p.value, accuracy = .0125, add_p = TRUE)`) as well as for $\alpha$ (r=`r cor.1v2.a$estimate`, df=`r cor.1v2.a$parameter`, *p* < `r scales::pvalue(cor.1v2.a$p.value, accuracy = .0125, add_p = TRUE)`). The results of TOSTs for $Q_0$ and $\alpha$ were non-significant (`r scales::pvalue(tost.1v2.q0$tost.p.value, accuracy = .0125, add_p = TRUE)`) and significant (`r scales::pvalue(tost.1v2.a$tost.p.value, accuracy = .0125, add_p = TRUE)`), respectively. That is, the results of the two strategies appeared to provide statistically equivalent estimates for $\alpha$ but not for $Q_0$.
...
Evaluations of the relationship between Strategy 2 and 3 revealed perfect correlations for $Q_0$ (r=`r cor.1v3.q0$estimate`, t=`r cor.1v3.q0$statistic`, df=`r cor.1v3.q0$parameter`, `r scales::pvalue(cor.1v3.q0$p.value, accuracy = .0125, add_p = TRUE)`) as well as for $\alpha$ (r=`r cor.1v3.a$estimate`, df=`r cor.1v3.a$parameter`, *p* < `r scales::pvalue(cor.1v3.a$p.value, accuracy = .0125, add_p = TRUE)`). Similarly, the results of TOSTs for $Q_0$ (`r scales::pvalue(tost.1v3.q0$tost.p.value, accuracy = .0125, add_p = TRUE)`) and $\alpha$ (`r scales::pvalue(tost.1v3.a$tost.p.value, accuracy = .0125, add_p = TRUE)`) were significant overall. That is, the results of the two strategies appeared to produce statistically equivalent model parameters across both $Q_0$ and $\alpha$.

# Discussion

The Operant Demand Framework is increasingly applied to various areas of clinical and significance [@hursh2013behavioral; @reed2013behavioral]. Furthermore, models of operant demand are increasingly support in a variety of statistical packages and tools [@kaplan2019r; @gilroy2018demand]. Within each of these applications and tools, a variety of modeling options are available with few firm guidelines for determining which is best suited for each scenario. The purpose of this technical report was to review the pragmatic and mathematical underpinnings of two variants derived from the framework of @hursh2008economic--the Exponential [@hursh2008economic] and the Exponentiated [@koff2015expt] models of operant demand.

This report provides an in-depth review of how 0 consumption values have thus far been incorporated in evaluations of operant demand. That is, neither the Exponential and Exponentiated models may character demand at 0 and both are restricted to $A_{Lower}$ in the same manner. Furthermore, controlling for how error is minimized, fitting the Exponentiated model is statistically equivalent to optimizing the Exponential model using absolute error and replacing 0 values with $A_{Lower}$. Indeed, results of computer simulation and comparisons using a data generating process used in the @koff2015expt study confirm that the two models are essentially equivalent and that neither can characterize demand at 0, just at $A_{Lower}$.

## Splitting Hairs in Operant Demand Models
...
Note: Euler's constant raised to the power of $-\infty$ equates to a value of 0. That is, $e^{-\infty}=\frac{1}{e^\infty}=\frac{1}{\infty} \approx 0$. This is has effect of making the value in parenthesis equal to $-1$, which in turn results in the full subtraction of quantity $k$ from $log_{10}Q_0$. Similar to $A_{UPPER}$, this value may be raised to the linear scale as follows: $10^{log_{10}Q_0 - k}$.
