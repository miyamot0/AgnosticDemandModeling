...
```{r fig4, fig.cap="Comparisons of Modeling Strategies", fig.height=5.5}
...
An evaluation of the relationship between Strategy 1 and 2 revealed strong correlations for $Q_0$ (r = `r cor.1v2.q0$estimate`, t = `r cor.1v2.q0$statistic`, df = `r cor.1v2.q0$parameter`, *p* < `r cor.1v2.q0$p.value`) as well as for $\alpha$ (r = `r cor.1v2.a$estimate`, t = `r cor.1v2.a$statistic`, df = `r cor.1v2.a$parameter`, *p* < `r cor.1v2.a$p.value`). The results of TOSTs for $Q_0$ and $\alpha$ were non-significant (*p* = 0.244) and significant (*p* < 2.2e-16), respectively. That is, the results of the two strategies appeared to provide statistically equivalent estimates for $\alpha$ but not for $Q_0$.

## Strategy 1 vs. Strategy 3

```{r strat1vsstrat3, include=FALSE}
### TODO: Addin in
...
Evaluations of the relationship between Strategy 2 and 3 revealed nearly perfect correlations for $Q_0$ (r = 1, t = 82678, df = 74, *p* < 2.2e-16) and $\alpha$ (r = 0.9999, t = 22.515, df = 74, *p* < 2.2e-16). Similarly, the results of TOSTs for $Q_0$ (*p* = 1) and $\alpha$ (*p* = 1) were significant overall. That is, the results of the two strategies appeared to produce statistically equivalent model parameters across both $Q_0$ and $\alpha$.
