...
The Operant Demand Framework is increasingly applied to various issues of clinical and societal significance [@hursh2013behavioral; @reed2013behavioral]. Furthermore, formal models of operant demand are increasingly supported across a range of statistical packages and tools [@kaplan2019r; @gilroy2018demand]. Within these applications and tools, several modeling options are available yet few firm guidelines exist with which to assist researchers in navigating among the available options. The purpose of this technical report was to review the pragmatic and mathematical underpinnings of the two prevailing models derived from the framework of @hursh2008economic--the Exponential [@hursh2008economic] and the Exponentiated model [@koff2015expt]. The goals of this report were three-fold: First, one of the goals of this report was to review the behavior of the lower limit ($A_{LOWER}$) for both the Exponential and Exponentiated as its relates to the presence and absence of 0 consumption values. Second, and building from the first, the two models are largely interchangeable when the $A_{LOWER}$ limit is known and differences in and error weighting are negated. Third, this report was constructed in the hopes that this clarifies and affirms the varying strategies that have been evaluated across labs applying the Operant Demand Framework.

Regarding the first point, this report provides an in-depth discussion and review of how 0 consumption values have, thus far, been included in models directly derived from the @hursh2008economic framework. In sum, neither the Exponential nor the Exponentiated models can characterize demand at 0 and both are equally restricted to the same non-zero lower asymptote, $A_{Lower}$. Regardless of whether 0 consumption values can be included in the regression, this mathematically cannot take place as currently designed. As such, it warrants noting that neither model provides a complete solution to the absence of a true zero-asymptote. This is inherently due to the underlying log scale and alternative scales have been suggested should a true-zero asymptote be relevant to the research question [@gilroy2021zbe].

The second aim of this study was to provide a mathematical basis and justification for why the two different models should not be so strongly distinguished. That is, controlling for differences in scale and lower asymptotes, it is trivial to arrive at the same results for the Exponential and Exponentiated model even in the presence of 0 consumption values. The results of computer simulation using the data generating process featured in the original @koff2015expt study confirmed that the two models provide statistically equivalent results when controlling for the inherent differences between each implementation. However, it warrants clearly stating that this claim is not made in the service of favoring either form as the de facto standard for use in the Operant Demand Framework. Rather, the true boon here is in finding that these two historically opposing perspectives are more alike than they are different. As such, it is the hope of the author that future efforts are directed towards better understanding the properties models rather than reinforce any prior bias towards or against any particular model.

Lastly, the final goal of this report was to clearly articulate the final truth as it relates to various models derived from the @hursh2008economic framework. That is, proponents of each models were correct, for different reasons, and that both models were equally useful in extending the application and utility of the Operand Demand Framework. Indeed, minor modifications can be performed to make one model behavior like the other, and likewise, the reverse is also true. Additionally, it warrants praising the proponents of the @koff2015expt in their efforts to address the historically problematic issue of removing data not occurring at random. For decades, a significant portion of otherwise valid consumption data was never carried forward into formal analyses for proper study. 
...
The goal of this report is neither to discourage the use of the @koff2015expt nor sing the praises of the original @hursh2008economic model. Rather, this is written such that all of the referenced works have advanced the scope and breadth of the Operant Demand Framework. Indeed, strong preferences towards any specific model invite the potential for bias and researchers should carefully measure their analytical strategies against others based on how well they fit the research question(s). 
...
## Hursh & Silberburg (2008)

### Proof of $A_{UPPER}$ at $P = 0$
...
### Proof of $A_{LOWER}$ at $\displaystyle{\lim_{P \to \infty} f(x)}$
...
$$
\begin{aligned}
log_{10}A_{Lower} = log_{10}A_{Upper} - k
\end{aligned}
$$
